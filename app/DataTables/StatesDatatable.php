...
class StatesDatatable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addColumn('action', 'adminlte.states.datatables.action')
            ->addColumn('checkbox', 'adminlte.states.datatables.checkbox')
            ->rawColumns(['action', 'checkbox']);
    }

    public function query(State $model)
    {
        return $model->locale()->with(['city' => function($q) { $q->locale(); }])->newQuery();
    }
        ...
        return $this->builder()
            ...
            ->buttons(
                Button::make('create')
                    ...
                    ->text('<i class="fa fa-plus"></i> ' . __('dataTables.buttons.create state')),
                Button::make('print')
